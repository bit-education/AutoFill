...
def main(html: str, student_id: str):
    # 前端解析与AI labeling
    print(f"[INFO] start parsing the html front end...", end="", flush=True)
    parse_res = parse_html(html=html, ai_api_key=CONFIG["ai_api_key"], is_file=False)
    parse_out = copy.deepcopy(parse_res)
    print("finished!")
    # 数据库字段与前端匹配
    print(f"[INFO] start matching with db keys...", end="", flush=True)
...
def url():
    ...
    if request.method == 'POST':
        ...
        if identifier not in hash_name.keys():
            print(f"[INFO] Adding url hash name, name: {url_name}, url hash: {identifier}")
            hash_name[identifier] = url_name
            json.dump(hash_name, open(URL_HASH_FILE, "w", encoding="utf-8"), indent=4)
            return jsonify({"message": "success"}), 200
        else:
            print(f"[INFO] url hash name exists! name: {url_name}, url hash: {identifier}")
...
def submit():
    ...
    html_source = str(data.get('html_source'))  # 学生申请页面的html源码
    
    print(f"[INFO] executing autofill ... getting student_id: {student_id}, url hash id: {name}, length of html: {len(str(html_source))}")
    ...
    if cache_dir.exists() and (cache_dir / "parse.json").exists() and (cache_dir / "match.json").exists(): # page has been cached before
        print(f"[INFO] this page ({name}) has been cached! Loading parsing and matching data from cache...")
        with open(cache_dir / "parse.json", "r") as f:
            parse_res = json.load(f)
        with open(cache_dir / "match.json", "r") as f:
            matches = json.load(f)
        # load student data from db
        print(f"[INFO] loading data from db...")
        ...
        print(f"[INFO] this page ({name}) is not cached! start parsing and matching, will take about 2mins...")
    ...
    print(f"[INFO] filling user data to html...")
    ...
    print(f"[INFO] final fill set: {fill_set}")
    ...
    return jsonify({"filled_set": fill_set, "parsed_fields": matches, "students_fields": db_fields})
